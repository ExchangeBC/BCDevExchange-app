...
module.exports = function (app, db, passport) {
  ...
  app.get('/api/resources-sources', function (req, res) {
    ...
    for (var x in config.catalogues) {
      ...
      listOfCatalogues.push({
        'name': catalogue.name,
        'url': '/api/resources/' + catalogue.short_name.toLowerCase(),
        'short_name': catalogue.short_name
      })
    }

    res.send({
      "sources": listOfCatalogues
    })
  })
  ...
  app.get('/api/resources/:source/url/:url', function (req, res) {
    ...
    if (!req.params.url) {
      res.send(400, "Missing url parameter.")
      return
    }
    if (req.params.source !== "GitHub") {
      res.send(400, "At this time, source must be GitHub.")
      return
    }
